...
// A etiqueta da caixa deve acompanhar a escala dos controles do polígono; a antiga
// tarja de 31 unidades dominava visualmente anotações pequenas.
function boxLabelWidth(name: string) {
  return Math.max(58, name.length * 7 + 18);
}
          ...
          <svg ref={svgRef} viewBox="0 0 1000 650" preserveAspectRatio="none" onPointerDown={canvasPointerDown} onPointerMove={canvasPointerMove} onPointerUp={canvasPointerUp} onPointerCancel={canvasPointerUp} onAuxClick={(event) => event.preventDefault()} onContextMenu={finishDrawingWithRightClick} onDoubleClick={() => { if (tool === "polygon") finishPolygon(); if (tool === "line") finishLine(); }}>
            {visibleAnnotations.map((annotation) => {
              ...
              if (annotation.type === "line") return <g key={annotation.id}>
                ...
                <polyline className="line-shape" points={pointsToSvg(annotation.pts)} stroke={label.color} strokeWidth={lineThickness} />
            ...
            {draft && <rect x={draft.x} y={draft.y} width={draft.w} height={draft.h} fill={`${getLabel(activeLabel).color}25`} stroke={getLabel(activeLabel).color} strokeWidth={lineThickness} />}
            {polygonDraft.length > 1 && <g><polyline className="line-shape" points={pointsToSvg(polygonDraft)} fill="none" stroke={getLabel(activeLabel).color} strokeWidth={lineThickness} />{polygonDraft.map((coordinate, index, points) => index % 2 === 0 ? <circle className={index === 0 && polygonDraft.length >= 6 ? "polygon-close-point" : ""} key={index} cx={coordinate} cy={points[index + 1]} r={markerRadius * (index === 0 && polygonDraft.length >= 6 ? 1.35 : 1)} fill={getLabel(activeLabel).color} stroke="#fff" strokeWidth={markerRadius * .32} onPointerDown={index === 0 && polygonDraft.length >= 6 ? (event) => { event.preventDefault(); event.stopPropagation(); finishPolygon(); } : undefined} /> : null)}</g>}
            {lineDraft.length > 0 && <g>{lineDraft.length > 2 && <polyline className="line-shape" points={pointsToSvg(lineDraft)} stroke={getLabel(activeLabel).color} strokeWidth={lineThickness} />}{lineDraft.map((coordinate, index, points) => index % 2 === 0 ? <circle key={index} cx={coordinate} cy={points[index + 1]} r={markerRadius} fill={getLabel(activeLabel).color} stroke="#fff" strokeWidth={markerRadius * .32} /> : null)}</g>}
            ...
            {reshapeDraft.length > 1 && <g><polyline className="reshape-line" points={pointsToSvg(reshapeDraft)} stroke={getLabel(activeLabel).color} strokeWidth={lineThickness} />{reshapeDraft.length >= 4 && <><circle className="reshape-endpoint" cx={reshapeDraft[0]} cy={reshapeDraft[1]} r={markerRadius * 1.25} strokeWidth={markerRadius * .42} /><circle className="reshape-endpoint" cx={reshapeDraft.at(-2)} cy={reshapeDraft.at(-1)} r={markerRadius * 1.25} strokeWidth={markerRadius * .42} /></>}</g>}
